...
\begin{itemize}[nosep]
  \item Understand analog-to-digital conversion principles including sampling, quantization, and resolution.
  \item Recognize the architecture of the TM4C123 ADC modules including dual 12-bit converters and shared input channels.
  \item Configure ADC sample sequencers (SS0-SS3) with different depths and trigger sources.
  \item Configure GPIO pins for analog input using AFSEL, AMSEL, and DEN registers.
  \item Implement software-triggered and hardware-triggered ADC conversions.
  \item Read and interpret ADC conversion results from FIFO registers.
  \item Convert digital ADC values to physical quantities such as voltage and temperature.
  \item Implement both polling and interrupt-driven ADC operation modes.
  \item Utilize hardware averaging and multi-channel sampling for improved measurement accuracy.
\end{itemize}
